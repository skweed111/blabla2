...
//функции и все прочее чекай в main.ts
////
//Promice(промис)-это объект в JS поедставляющий собой 'обещание' выполнить некоторую операцию.

//Промис может находиться в одном из трех состояний:

//Pending(ожидание)-начальное состояние,промис еще не выполнен и не отклонен

//Fufilled(выполнено)-операция успешно завершена

//Rejected(Отклонено)-произошла ошибка и промис не может быть выполнен пример ниже

// const myPromise = new Promise((resolve, reject) => {
//   const success = true;
//   if (success) {
//     resolve("операция успешна выполнена");
//   } else {
...
/////////////
//new Promice()
//new - это оператор,который используется для создания экземпляра объекта на основе конструктора или класса
//конструкция new Promise() используется для создания нового объекта Promise,
// который представляет собой завершенное,либо незавершенное (еще в процессе выполнения) асинхронное действие
//чаще всего используется в качестве возвращаемого объекта асинхронной функции после оператора return
//конструктор Promise принимает функцию-исполнитель(executor) с двумя параметрами:resolve и reject
//эти параметры-будут вызваны для сигнализации об успешном или не успешном завершении промиса
//в них можно передавать как переменные так и текст
//В reject чаще всего либо текст об ошибке,либо экземпляр объекта ошибки new Error
//////
//промис является объектом который возвращает 3 результата
//then - срабатывает если промис завершен успешно и возвращает полученный ответ
//
//catch - срабатывает если промис завершен reject и возвращает передаваемый ответ
//
//finaly - срабатывает всегда после выполнения вне зависимости от результата
//
//в then передается функция,параметр которой является ответ response (Response — это объект, который представляет собой ответ на HTTP-запрос.
// Он используется в веб-разработке для работы с данными, полученными от сервера. В JavaScript и TypeScript объект Response часто встречается при работе с API,
//  например, с помощью fetch или в сервис-воркерах.)
//
//в  catch передается функцтя параметр которой является ошибка error
//
//в finally передается callback
////

// функция fetch это -api для совершения http запросов
// она возвращает промис которая возврощает ответ Response
// при успешном совершениизапроса
// раз она возвращает промис то да эта функция является по себе асинхронной
// синтаксис:
// fetch(params).then().catch().finally()-все то же замое что и в обычной асинхронной функции

// url - ссылка на эндпоинт. это основной параметр. Запрос уже
// можно сделать просто написав его, к примеру:
// fetch(“url”).then(response => {})
//  options - необязательный параметр по дополнительной настройке,
// где можно указывать метод (GET/POST/PUT/DELETE), заголовки (тип авторизации,
// временная зона и т.д), тело (данные которые передаем на сервер)
//пример:
//fetch("",method:"GET",headers:{},body:JSON.stringify(user) });
//////////////
// все простые типы данных
// (Number, String, Date и т.д) в js являются классами.
// Если при работе с числами и строками мы не использовали классы
// напрямую, то с типом Date работа происходит по другому.
// Когда мы хотим сформировать значение даты, то необходимо
// создавать экземпляр класса даты new Date()
// Как и с обычным классом в него также можно передать параметр, в
// new Date(params) - params будет значение в формате*, таким
// образом можно получить дату с определенным временем
///////////////////методы ниже:
// Private-используется для определения приватных свойств класса
//
//Get -это ключевое слово для создания геттеров getInfo(),getData(),getHui()
//геттеры- это спец.методы,которые позволяют получать значения определнного свойства объекта.Они
//предоставляют доступ к свойствам,но не позволяют их изменять.
//
//Set-используется для создания сеттеров
//сеттеры-это это методы которые принимают значения и задают это значение свойству
//только в отличии от обычных методов,значение передается не как параметр,а как если бы это была обычная переменная \
